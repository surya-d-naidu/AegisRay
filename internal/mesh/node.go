...
func NewMeshNode(cfg *config.MeshConfig) (*MeshNode, error) {
	...
	// Load or generate long-term RSA identity key
	...
	if cfg.IdentityKeyFile != "" {
		...
		if err != nil {
			...
			// Key doesn't exist, will be generated by EncryptionManager
		} else {
			rsaKey = loadedKey
		}
	}
	...
	encryption, err := crypto.NewEncryptionManager(rsaKey)
	if err != nil {
	...
	if rsaKey == nil && cfg.IdentityKeyFile != "" {
	...
	node.httpServer = NewHTTPServer(node, cfg.APIPort)
...
func (n *MeshNode) handleKeyExchangePacket(packet *MeshPacket) {
	...
	// 1. Verify signature using peer's RSA public key (Authenticity)
...
func (n *MeshNode) sendEphemeralKey(peer *Peer, privKey *ecdh.PrivateKey) {
	...
	// 1. Sign our public key with our RSA private key (Authentication)
